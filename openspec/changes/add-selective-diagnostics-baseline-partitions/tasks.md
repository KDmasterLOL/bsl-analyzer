...
- [ ] 1.1 Добавить `include` в `DiagnosticsBaselineConfig`, default-all и точную
...
- [ ] 1.2 Расширить существующий `DiagnosticsBaselinePartitionPlan` enabled id/policy и
...
- [ ] 1.3 Реализовать versioned canonical selection fingerprint отдельно от diagnostic
...
- [ ] 2.1 Специализировать schema-v1 manifest loader: enabled identity validation,
...
- [ ] 2.2 Ограничить observations enabled objects, сохранить capability/no-follow и
...
- [ ] 2.3 Добавить в общий classifier `unsuppressed`, сохранив общий fingerprint и
  защитные diagnostics как active `new`. (SDBP-03; Decision 2) Проверка:
  `cargo test -p ide selective_baseline_classification`.
- [ ] 2.4 Расширить coverage/summary отдельными policy, Full/Partial state и counts без
...
- [ ] 3.1 Реализовать all/selected CLI matrix, global Full, selected result scope,
...
- [ ] 3.2 Сохранить atomic manifest transaction, carry-through dormant metadata,
...
- [ ] 3.3 Реализовать streaming `create --from-v1` для всех enabled owners,
...
- [ ] 3.4 Реализовать config-only переход full manifest, dormant carry-through при
...
- [ ] 4.1 Подключить общий policy classifier после одного полного post-suppression
...
- [ ] 4.2 Расширить shared summary и console/goldens; сохранить JSON/JSONL/SARIF/JUnit
...
- [ ] 5.1 Повысить MCP schema/outputSchema до 15, добавить file/workspace policy,
...
- [ ] 5.2 Расширить единый линейный MCP response fitter на mixed-policy success и
  enabled-error envelope. (SDBP-08; Decision 8) Проверка:
  `cargo test -p mcp-server --test selective_diagnostics_baseline`.
- [ ] 5.3 Реализовать LSP mixed-policy publication, enabled fail-visible error и
...
- [ ] 5.4 Провести изменение `include` через штатный config reload; сохранить no-Salsa
  baseline-file reload, batch reset, enabled watches и dormant silence. (SDBP-09;
  Decision 8) Проверка:
  `cargo test -p bsl-analyzer --test selective_diagnostics_baseline_lsp`.
- [ ] 5.5 Доказать CLI/MCP file/workspace/LSP parity общего owner, fingerprint, policy и
...
- [ ] 6.1 Добавить adversarial enabled path/symlink/reparse regressions, переиспользуя
...
- [ ] 6.2 Добавить парный ignored release gate selective/full load на 1,6 млн с
...
- [ ] 6.3 Добавить парный ignored release gate selective/full v1 migration на 1,6 млн
  ...
  workflow `.github/workflows/ci.yml`, job `partitioned-baseline-scale`.
- [ ] 6.4 Обновить только `docs/configuration/DIAGNOSTICS.md`,
...
- [ ] 6.5 Добавить механический inventory test, который находит change в active либо
...
- [ ] 6.6 Выполнить целевые шлюзы: `cargo fmt --all -- --check`;
...
- [ ] 6.7 Выполнить полный Linux/Windows/OpenSpec gate:
