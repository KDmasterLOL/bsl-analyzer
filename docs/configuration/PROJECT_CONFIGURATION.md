...
отклоняются до начала анализа. **Семантика видимости по зависимостям ещё не
применяется**: конфигурация с непустым `dependsOn` пока отклоняется целиком
с явной ошибкой, чтобы не анализировать проект с видимостью, отличной от
заявленной. Записи без `dependsOn` можно использовать уже сейчас — например,
ради стабильных имён.
