...
- **`root_id` — часть тождества находки, а не украшение.** Расширение повторяет структуру
  ...
  корнями, и различает их только `root_id`. У конфигурации он пустой, у расширения — его
  идентификатор относительно каталога проекта. В текстовой раскладке непустой идентификатор
  печатается пометкой перед путём. Появление этого поля и подняло версию до `2`.
