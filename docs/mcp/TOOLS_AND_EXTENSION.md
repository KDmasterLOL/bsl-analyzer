...
| `workspace` | `graph` | граф вызовов: `overview`, `schema`, `status`, `node`, `source`, `neighbors`, `callers`, `callees`, `resolve`; виды рёбер включают строковую диспетчеризацию `notify_ref` (`ОписаниеОповещения`) и `idle_handler` (`ПодключитьОбработчикОжидания`) с провенансом `string_resolved` | `--source-dir` |
