...
`execute(action=run|eval)` по умолчанию запрещён. Для расходуемой тестовой ИБ
нужно одновременно установить `allow_execute: true` в реестре и назначить
пользователю роль `BSL_ВыполнениеКода`. `execute(action=check)` и операции чтения
используют `BSL_ОсновнаяРоль`.
