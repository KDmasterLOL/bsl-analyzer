...
- `--allowed_hosts` принимает массив имён и адресов сервера, по которым
  ...
  Пустое значение не считается
  разрешённым именем и отвергается: иначе сервер запустился бы и отказывал всем.
  Заданный список заменяет встроенные локальные адреса целиком, поэтому для
  доступа с самой машины перечислите `localhost`, `127.0.0.1` и `::1` явно.
  Адрес привязки
  `0.0.0.0` (или `::`) сюда не годится: клиент шлёт в `Host` тот адрес, по
  которому подключался, а не адрес прослушивания, поэтому такая запись не
  разрешает почти ничего — при старте о ней предупреждает лог.
- `--config` принимает те же три настройки в полях `host`, `port` и массиве
  `allowed_hosts`. Его нельзя сочетать с отдельными HTTP-параметрами;
