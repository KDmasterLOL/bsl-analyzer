...
Каталог кеша принадлежит одному воркспейсу: при первом запуске в нём создаётся
отметка `workspace-owner`, и попытка отдать тот же каталог другому `--source-dir`
завершается ошибкой запуска. Общий кеш на несколько конфигураций не поддержан —
графы и поисковые индексы разных проектов затирали бы друг друга. Каждому
проекту нужен свой каталог.
