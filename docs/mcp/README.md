...
```bash
bsl-analyzer mcp serve --profile workspace --source-dir ./my-project \
  --enable-tool references --enable-tool call_hierarchy
```

Ключ повторяемый, имена — те же, что в `tools/list`. Имя, которого профиль не
объявляет, останавливает старт и печатает список допустимых; молча отброшенного
ключа не бывает. Назвать инструмент, который профиль и так отдаёт, разрешено и
ничего не меняет — благодаря этому конфиг с ключом продолжит работать в релизе,
где инструмент станет умолчанием.
...
```bash
bsl-analyzer mcp install --target claude --preset workspace --enable-tool references
```
