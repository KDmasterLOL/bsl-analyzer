...
Каталог кеша принадлежит одному воркспейсу. При первом запуске с `--cache-dir` в
нём создаётся отметка `workspace-owner`, и запуск завершается ошибкой, если
каталог уже занят: в нём лежит отметка другого `--source-dir` либо уже собранные
`bsl-graph.db`/`bsl-search.db` без отметки — так распознаётся `.build` соседнего
проекта, ведь умолчание отметку не ставит. Пустой каталог без отметки считается
свободным, а свой собственный `<source-dir>/.build` можно указать явно.

Общий кеш на несколько конфигураций не поддержан: графы, поисковые индексы и лиз
писателя у разных проектов затирали бы друг друга. Каждому проекту нужен свой
каталог.
