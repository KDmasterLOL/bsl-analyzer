...
Чтобы запустить любую из описанных выше конфигураций по HTTP, добавьте
`--mode http` и обязательный параметр `--port` со значением от `1` до `65535`.
...
- `--host` задаёт адрес сервера; по умолчанию используется `127.0.0.1`;
- `--allowed-host` задаёт разрешённое значение заголовка `Host` и обязателен
  для нелокального адреса;
- `--host`, `--port` и `--allowed-host` используются только с `--mode http`.

MCP доступен по адресу `http://<host>:<port>/mcp`. Одинаковые HTTP-процессы
одновременно не запускаются. Без `--mode http` поведение команды не меняется.
