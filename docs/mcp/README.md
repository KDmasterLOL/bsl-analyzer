...
```bash
bsl-analyzer mcp serve --profile workspace --source-dir ./my-project \
  --enable-tool <имя-из-opt_in_tools>
```

**В текущей сборке опт-ин инструментов нет** — `opt_in_tools` в декларации пуст
у обоих профилей, включать пока нечего, и любое подставленное имя будет либо
отвергнуто, либо назовёт инструмент, который и так обслуживается. Механизм
описан здесь потому, что им включаются инструменты, которые появятся позже;
действующий список всегда брать из декларации, а не отсюда.
