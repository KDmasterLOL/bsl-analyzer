# Provenance: NumberOfOptionalParams
...
number of optional parameters is a concrete local heuristic derived from that
guidance.
...
- HIR lowering computes the number of optional parameters for a method;
- the handler compares that count with the configurable
...
- methods with no optional parameters;
...
`NumberOfOptionalParams` is a strong permissive candidate because it is a local
