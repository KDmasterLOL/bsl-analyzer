# Provenance: CommonModuleNameClient
...
The rule is organizational and metadata-based: a non-global client common
module should expose its client-side role in the module name through the
`Клиент` / `Client` postfix.
...
- it checks that the module is client-side and non-global;
- it validates the module name against local keyword variants;
- it reports only when the client-module naming convention is violated.
...
`#std469` and client-module naming semantics rather than inherited short-form
wording.
...
- client module without postfix;
- client module with postfix;
- global module exception;
...
`CommonModuleNameClient` is a good permissive candidate because:
