...
| Virtual table arguments — nested function call + named-condition trailing arg | pubqlang chapter 104 line 23 | verified yes (C2 of Slice 8-addendum — `РегистрНакопления.Продажи.Обороты(&НачалоПериода, КОНЕЦПЕРИОДА(&КонецПериода, ДЕНЬ), , Номенклатура = &Товар) КАК ПродажиОбороты` confirmed verbatim — date-helper function call `КОНЕЦПЕРИОДА(...)` as an arg + empty-middle-slot + named-condition `Номенклатура = &Товар` trailing arg); ITS Tier A1 (primary structural for "expression-as-arg with nested function call") |
| Virtual table arguments — `Обороты()` parameter-order prose | pubqlang chapter 116 §Параметры виртуальной таблицы оборотов | verified yes (C2 of Slice 8-addendum — `Параметры виртуальной таблицы оборотов позволяют задать условие отбора данных из информационной базы. Параметры следует задавать строго в порядке их описания: НачалоПериода, КонецПериода, Периодичность, Условие, КорСубконто, ...`); cited via §section name per D8a line-number-stability fallback after sample lines 13/36 in the local mirror were checked and the section anchors are stable across paginated rendering. ITS Tier A1 (primary positional-args attestation) |
| Virtual table arguments — empty `Остатки()` and leading-empty `Остатки( , cond)` | pubqlang chapter 152 lines 23, 35 | verified yes (C2 of Slice 8-addendum — line 23 `РегистрНакопления.ТоварыНаСкладах.Остатки() КАК ТоварыНаСкладахОстатки` no-args form; line 35 `РегистрНакопления.ТоварыНаСкладах.Остатки( , Номенклатура = &Номенклатура) КАК ТоварыНаСкладахОстатки` leading-empty + named-condition form); ITS Tier A1 (primary attestation for IDE-recovery allowances #4 and #1) |
