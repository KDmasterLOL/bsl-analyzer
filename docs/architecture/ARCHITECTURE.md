...
и не кончается пробелом, переводом строки или комментарием, а
`node.text_range()` идёт ровно от первого значимого токена узла до последнего.
Корень исключён по необходимости — хвост файла держать больше негде.

Наружу уходит только краевая тривия. Внутренняя остаётся: у `Т . Поле` пробелы
стоят между значимыми токенами одного узла, и `node.text()` отдаст их вместе с
именем. Составное имя из `node.text()` собирать по-прежнему нельзя — для этого
есть `ast_utils::direct_name_tokens`.
...
- узел без единого значимого токена пуст, и такие узлы (обычно `ERROR`
  ...
  об пустой узел спотыкается — шаг назад делает
  `syntax::prev_token_past_empty`;
- курсор в хвостовой тривии файла стоит уже в корне, а не в узле, который
  печатают: конструкции без закрывающего слова кончаются на последнем
  набранном токене. Кто ищет объемлющий узел по предкам токена под курсором,
  обязан это учесть.
...
привязки не закрывается и живёт отдельно — см.
[`adr/ADR-02-line-sensitivity.md`](adr/ADR-02-line-sensitivity.md).

Тем же промежутком между словами распоряжается и третья норма — куда указывает
диагностика разбора. Она о другом: привязка тривии отвечает, какому узлу
достанется пробел, а эта — на какое место в тексте показать, когда ошибка
сообщает о токене, которого в тексте нет. См.
[`adr/ADR-03-error-range-attribution.md`](adr/ADR-03-error-range-attribution.md).
