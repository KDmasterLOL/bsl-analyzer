...
"#;
    ...
    assert_eq!(var_ty(&db, file_id, "х"), Some(Ty::String));
}
...
"#;
    let (db, file_id) = setup(fixture);
    assert_eq!(var_ty(&db, file_id, "х"), Some(Ty::Number));
}
...
"#;
    ...
    assert!(
        members.iter().any(|m| matches!(m, Ty::String)),
        "union must contain Ty::String, got {members:?}",
    );
    assert!(
        members.iter().any(|m| matches!(
            m,
            Ty::MetadataRef { kind: MetadataKind::CatalogRef, name } if name.as_str() == "Справочник1"
        )),
...
"#;
    ...
    assert!(
        mm.iter().any(|(e, a)| matches!(e, Ty::Number) && matches!(a, Ty::String)),
