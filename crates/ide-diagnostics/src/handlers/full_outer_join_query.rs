...
pub fn check(ctx: &DiagnosticsContext) -> Vec<Diagnostic> {
    crate::sdbl_utils::collect_sdbl_simple(
        ...
        "Using FULL OUTER JOIN significantly reduces query performance. Consider rewriting using UNION with LEFT JOIN",
        |diag| match diag {
            sdbl_hir::SdblDiagnostic::FullOuterJoin { range } => Some(*range),
            _ => None,
        },
    )
}
