...
КонецФункции"#;
    ...
    fn test_high_complexity_triggers_diagnostic() {
        // Function with complexity 21 (threshold 20) — should trigger
        // Decision points: Если(1) + ИначеЕсли(1) + Иначе(1) + Если(1) + Если(1) +
        //   ИначеЕсли(1) + Иначе(1) + Для(1) + Если(1) + ИначеЕсли(1) + Иначе(1) +
        //   Для(1) + Если(1) + ИначеЕсли(1) + ИначеЕсли(1) + Иначе(1) +
        //   Если(1) + ?(1) + ?(1) + ?(1) + ИЛИ(1) = 21
...
КонецФункции"#;
        ...
        // Diagnostic points at function name "СерверныйМодульМенеджера" (col 8-32 on line 0)
