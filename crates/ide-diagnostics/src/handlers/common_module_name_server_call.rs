...
pub fn check(ctx: &DiagnosticsContext) -> Vec<Diagnostic> {
    if ctx.config.is_disabled(DiagnosticCode::CommonModuleNameServerCall) {
        return Vec::new();
    }
    ...
    let regex = Regex::new(r"(?i)вызовсервера|servercall").unwrap();
...
mod tests {
    ...
    fn check_with_module(code: &str, module: &bsl_metadata::CommonModule) -> Vec<Diagnostic> {
        ...
        let regex = regex::Regex::new(r"(?i)вызовсервера|servercall").unwrap();
        if regex.is_match(module.name()) {
            return Vec::new();
        }
