...
КонецФункции"#;
    ...
    // Mirror of CognitiveComplexityDiagnostic.bsl: function with complexity 82
    // (deeply nested if-chains over metadata object types)
    const COMPLEX_FUNCTION: &str = r#"Функция СерверныйМодульМенеджера(Имя)
	ОбъектНайден = Ложь;
