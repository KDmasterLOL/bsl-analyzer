...
/// Lazy per-method body lowering paired with its [`BodySourceMap`].
///
/// Sister query of [`method_body_query`] for IDE features that need
/// offset ↔ HIR node mapping (hover, narrow body-infer, diagnostics).
/// Shares the same residency contract (see module docs): no
/// `file_resident` gate; tracked text reads inside `db.parse` panic by
/// Salsa contract if the Phase O total-VFS invariant is violated.
///
/// Diagnostics are dropped intentionally — see module docs.
#[salsa::tracked(lru = 4096)]
