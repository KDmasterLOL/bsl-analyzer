...
//!
//! Поэтому направление держат три синтетических контроля в модуле тестов
//! `crates/parser/src/sink.rs` — по одному на вид, у которого выбор смещения
//! есть (`a_missing_token_error_after_buffered_trivia_points_at_the_next_word`,
//! `a_recovery_span_starting_on_trivia_begins_at_the_next_word`,
//! `a_bump_token_error_after_trivia_points_at_the_last_word`). Им нужен
//! событийный вектор с ошибкой на позиции пробела, а `Event` и `Sink` крейт
//! наружу не отдаёт. Все три были увидены падающими: `2..2` вместо `5..5`,
//! `2..5` вместо `3..5` и `2..5` вместо `0..2`.
