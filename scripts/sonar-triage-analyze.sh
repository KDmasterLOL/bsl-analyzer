default_low_analysis() {
  "$JQ_BIN" -n '{confidence:"low", classification:"unknown", summary:"Контекст неполный; зафиксировать факт закрытия диагностики для будущего разбора.", evidence:[], unknowns:["opencode analysis was skipped or returned invalid JSON"], recommended_gitlab_action:"create_issue"}'
}
...
analyze_issue() {
  ...
  local dir=$2
  local idx=$3
  local issue_file raw prompt parsed
  issue_file="$dir/opencode-input-$idx.json"
  "$JQ_BIN" -n --argjson issue "$issue" --arg repo_root "$REPO_ROOT" \
    '{task:"triage_closed_sonar_issue", repo_root:$repo_root, sonar_issue:$issue}' > "$issue_file"
...
analyze() {
  local dir issue parsed idx manifest_tmp
  dir=$(run_dir)
  [[ -f "$dir/issues.json" ]] || die "issues.json not found for run $RUN_ID"
  : > "$dir/analysis.ndjson"
  idx=0
  while IFS= read -r issue; do
    parsed=$(analyze_issue "$issue" "$dir" "$idx")
